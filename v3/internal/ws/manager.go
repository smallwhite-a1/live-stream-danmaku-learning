...
//
// Clients is a snapshot slice borrowed from Manager.snapshotPool. The worker
// must return it with releaseSnapshot after sending.
...
//
// V3 keeps V2's worker pool design, then optimizes the hottest allocation:
// building a []Client snapshot on every broadcast.
func (m *Manager) Run() {
...
func (m *Manager) releaseSnapshot(clients []*Client) {
	...
	// Clear pointer slots before putting the slice back.
	//
	// The backing array may otherwise keep old *Client references alive longer
	// than necessary, which makes GC work harder during churn.
	for i := range clients {
		clients[i] = nil
	}
