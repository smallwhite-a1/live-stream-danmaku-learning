...
import (
	...
	"github.com/charlesAcmen/livestream-danmaku/v3/internal/ws"
)
...
func main() {
	...
	addr := ":" + *port
	log.Printf("[server] V3 listening on %s workers=%d", addr, *workers)
...
func logMetrics(manager *ws.Manager) {
	...
	for range ticker.C {
		m := manager.Metrics()
		log.Printf("[metrics] rooms=%d clients=%d workers=%d bq=%d/%d jq=%d/%d packets=%d jobs=%d dropped_jobs=%d delivered=%d dropped_messages=%d pool_gets=%d pool_puts=%d pool_news=%d goroutines=%d alloc=%dKB total_alloc=%dKB gc=%d",
			m.Rooms,
			m.Clients,
			m.WorkerCount,
			m.BroadcastQueueLen,
