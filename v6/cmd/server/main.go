...
import (
	...
	"github.com/charlesAcmen/livestream-danmaku/v6/internal/queue"
	"github.com/charlesAcmen/livestream-danmaku/v6/internal/ws"
...
func main() {
	...
	brokersRaw := flag.String("brokers", getenv("V6_KAFKA_BROKERS", infra.DefaultKafkaBrokers), "Kafka broker list, comma separated")
	topic := flag.String("topic", getenv("V6_KAFKA_TOPIC", infra.DefaultKafkaTopic), "Kafka topic for danmaku persistence")
	redisAddr := flag.String("redis-addr", getenv("V6_REDIS_ADDR", infra.DefaultRedisAddr), "Redis address")
	flag.Parse()
	...
	go func() {
		log.Printf("[server] V6 listening on :%s workers=%d kafka=%v redis=%v", *port, *workers, *kafkaEnabled, *redisEnabled)
...
func logMetrics(manager *ws.Manager, publisher *queue.KafkaPublisher) {
	...
	for range ticker.C {
		m := manager.Metrics()
		if publisher == nil {
			log.Printf("[metrics] rooms=%d clients=%d workers=%d local_fanout=%d delivered=%d redis_pub=%d redis_recv=%d kafka=disabled goroutines=%d alloc=%dKB gc=%d",
				m.Rooms,
				m.Clients,
				m.WorkerCount,
		...
		log.Printf("[metrics] rooms=%d clients=%d workers=%d local_fanout=%d delivered=%d redis_pub=%d redis_recv=%d kafka_enqueued=%d kafka_dropped=%d kafka_errors=%d goroutines=%d alloc=%dKB gc=%d",
			m.Rooms,
			m.Clients,
			m.WorkerCount,
			m.LocalFanoutPackets,
