...
import (
	...
	"github.com/charlesAcmen/livestream-danmaku/v6/internal/consumer"
	"github.com/charlesAcmen/livestream-danmaku/v6/internal/infra"
	"github.com/charlesAcmen/livestream-danmaku/v6/internal/repo"
)

func main() {
	brokersRaw := flag.String("brokers", getenv("V6_KAFKA_BROKERS", infra.DefaultKafkaBrokers), "Kafka broker list, comma separated")
	topic := flag.String("topic", getenv("V6_KAFKA_TOPIC", infra.DefaultKafkaTopic), "Kafka topic")
	groupID := flag.String("group", getenv("V6_KAFKA_GROUP", infra.DefaultKafkaGroupID), "Kafka consumer group id")
	mysqlDSN := flag.String("mysql-dsn", getenv("V6_MYSQL_DSN", infra.DefaultMySQLDSN), "MySQL DSN")
