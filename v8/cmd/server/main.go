...
import (
	...
	"github.com/charlesAcmen/livestream-danmaku/v8/internal/ws"
...
func main() {
	...
	topic := flag.String("topic", getenv("V8_KAFKA_TOPIC", infra.DefaultKafkaTopic), "Kafka topic for danmaku persistence")
	redisAddr := flag.String("redis-addr", getenv("V8_REDIS_ADDR", infra.DefaultRedisAddr), "Redis address")
	flag.Parse()
	...
	go func() {
		log.Printf("[server] V8 listening on :%s workers=%d kafka=%v redis=%v", *port, *workers, *kafkaEnabled, *redisEnabled)
...
func logMetrics(manager *ws.Manager, publisher *queue.KafkaPublisher) {
	...
	for range ticker.C {
		...
		log.Printf("[metrics] rooms=%d clients=%d workers=%d local_fanout=%d delivered=%d stats=%d likes=%d online_reports=%d redis_pub=%d redis_recv=%d kafka_enqueued=%d kafka_acked=%d kafka_dropped=%d kafka_errors=%d goroutines=%d alloc=%dKB gc=%d",
			m.Rooms,
			m.Clients,
			m.WorkerCount,
			m.LocalFanoutPackets,
