...
import (
	...
	"github.com/charlesAcmen/livestream-danmaku/v8/internal/consumer"
	"github.com/charlesAcmen/livestream-danmaku/v8/internal/infra"
	"github.com/charlesAcmen/livestream-danmaku/v8/internal/queue"
	"github.com/charlesAcmen/livestream-danmaku/v8/internal/repo"
)

func main() {
	brokersRaw := flag.String("brokers", getenv("V8_KAFKA_BROKERS", infra.DefaultKafkaBrokers), "Kafka broker list, comma separated")
	topic := flag.String("topic", getenv("V8_KAFKA_TOPIC", infra.DefaultKafkaTopic), "Kafka topic")
	dlqTopic := flag.String("dlq-topic", getenv("V8_KAFKA_DLQ_TOPIC", infra.DefaultKafkaDLQTopic), "Kafka dead letter topic")
	groupID := flag.String("group", getenv("V8_KAFKA_GROUP", infra.DefaultKafkaGroupID), "Kafka consumer group id")
	mysqlDSN := flag.String("mysql-dsn", getenv("V8_MYSQL_DSN", infra.DefaultMySQLDSN), "MySQL DSN")
	...
	log.Printf("[consumer] started brokers=%v topic=%s dlq=%s group=%s batch=%d flush=%s",
		brokers, *topic, *dlqTopic, *groupID, *batchSize, *flushInterval)
...
func logMetrics(ctx context.Context, handler *consumer.Handler) {
	...
	for {
		...
		case <-ticker.C:
			m := handler.Metrics()
			log.Printf("[consumer metrics] parsed=%d saved=%d inserted=%d duplicates=%d batches=%d skipped=%d dlq=%d dlq_failures=%d retries=%d failed_batches=%d last_batch_ms=%d",
				m.Parsed,
				m.Saved,
				m.Inserted,
				m.Duplicates,
