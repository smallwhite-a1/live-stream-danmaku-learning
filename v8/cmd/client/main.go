...
func readLoop(conn *websocket.Conn, done chan<- struct{}) {
	...
	for {
		...
		case model.TypeStats:
			...
			if err := json.Unmarshal(packet.Data, &stats); err != nil {
				log.Printf("[client] invalid stats: %v", err)
				continue
			}
			log.Printf("[stats] online=%d likes=%d", stats.Online, stats.Likes)
		default:
...
func inputLoop(conn *websocket.Conn) {
	...
	for scanner.Scan() {
		text := scanner.Text()
		if text == "1" {
			payload, err := json.Marshal(model.Like{Count: 1})
			if err != nil {
				log.Printf("[client] marshal like failed: %v", err)
				continue
			}
			...
			if err := conn.WriteJSON(packet); err != nil {
				log.Printf("[client] send like failed: %v", err)
				return
			}
			log.Printf("[client] liked")
			continue
		}
