...
import (
	...
	"github.com/charlesAcmen/livestream-danmaku/v5/internal/consumer"
	"github.com/charlesAcmen/livestream-danmaku/v5/internal/infra"
	"github.com/charlesAcmen/livestream-danmaku/v5/internal/repo"
)

func main() {
	brokersRaw := flag.String("brokers", getenv("V5_KAFKA_BROKERS", infra.DefaultKafkaBrokers), "Kafka broker list, comma separated")
	topic := flag.String("topic", getenv("V5_KAFKA_TOPIC", infra.DefaultKafkaTopic), "Kafka topic")
	groupID := flag.String("group", getenv("V5_KAFKA_GROUP", infra.DefaultKafkaGroupID), "Kafka consumer group id")
	mysqlDSN := flag.String("mysql-dsn", getenv("V5_MYSQL_DSN", infra.DefaultMySQLDSN), "MySQL DSN")
