...
当前契约限制每个窗口最多保存 500 条代表事件、模型提示词最多 8000 个 Unicode 字符、单次领取最多 32 个到期窗口、默认任务队列容量为 128。它们分别限制内存、模型输入、单轮工作量和积压规模。

规则计数会在代表事件装满后继续累加，所以指标仍反映窗口总量；模型只读取确定性选出的代表样本。此取舍适合运营洞察：优先保证服务在突发流量下仍可运行和解释。
